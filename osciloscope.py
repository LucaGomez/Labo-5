...
class Osciloscope(object):
    ...
    def __init__(self, instrument_number = 1):
	#Defino el recurso
    ...
    def guardar(self, channel):
        t, V = self.getWindow(channel)
        ...
        # f = open(folderpath  + 'medicion.txt', 'a')
        # f.write(str(t) + "\n")
        # f.write(str(V) + "\n")
        # f.close()    
        np.savetxt(folderpath  + 'medicion_tiempo.txt', data_to_save)
    
    def Vpp(self):
        self._osci.query_ascii_values("MEAS3:TYPE PK2PK")
